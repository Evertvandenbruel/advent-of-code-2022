...
Monkey = Struct.new(:name, :items, :command, :divisible_by, :divisible_true, :divisible_false, :count, keyword_init: true) do
  ...
  def do_round(monkeys, multiply)
    while self.items.size > 0

      self.count = 0 if count == nil
      self.count += 1

      value = inspect_item(old: items.pop)
...
multiply = monkeys.map(&:divisible_by).inject(:*)

10000.times do |i|
  monkeys.each do |monkey|
    monkey.do_round(monkeys, multiply)
  end
end
